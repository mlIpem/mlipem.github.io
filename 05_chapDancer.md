...
In this chapter we look at what music does with dancers of classical ballet, while assuming their roles prescribed by the narrative.
The music is a constant given audio track and 
we show that repeats in the musical structure, such as repeats of musical fragments and phrases within fragments, somehow influence the micro-timing in dancing without them being aware of this influence.
...
A dance solo from the Swan Lake (1875–76) by Pyotr Ilyich Tchaikovsky is taken as an example for our study of micro-timing. The dance forms part of a narrative of Princess Odette's metamorphosis into a swan, orchestrated by a malevolent sorcerer. Accordingly, dancers articulate swan personas as their movements unfold with a precise swan-like alignment to the musical cadence. The dance solo known as *Odile variation* was recorded, forming the starting point for studying the effect of phrase repeats on the micro-timing in dance-music synchronizations. The data have been analysed in Tabeli et al. (2023).

The synchronization of body movement to a musical beat is a sensory-motor process, guided by brain predictions about timing. At the micro-timing level, measured in milliseconds and centiseconds, synchronization is primarily automated, although skilled performers can exert a top-down influence. For instance, experienced jazz musicians can intentionally play *laid-back* or *up-beat*, affecting micro-timing by introducing varying levels of anticipation in synchronization with fellow musicians. Given that context, our focus is whether micro-timing can be independently influenced by the mere repetition in musical structure, devoid of intentional or even vaguely intentional acts. 

The mechanisms underlying the potential influence on micro-timing are likely associated with memory, anticipation, and expression. Repetition in choreographic sequences is intricately linked to memory and anticipation/prediction frameworks. The theory states that through training, movement sequences become stored in motor memory. Accordingly, cognitive effort required for their activation during performance is reduced. However, the precise dynamics of the interaction between the retrieval of high-level (cognitive) sequences and their execution at the low-level (sensory-motor) synchronized micro-timing may hinge on the effectiveness of the established prediction model.

The work hypothesis is that a sequence establishes a timing model in the dancer. When the sequence is repeated shortly after, the established timing model facilitates the timing for that sequence, which translates to a subtle increase in anticipation. It is likely that a dancer intuitively uses this effect as an expressive element to drive and create tension. In line with our theory of self-augmented music interaction, dancing along with the music could create the illusion of agency, that dancing causes music. This illusion may generate arousal which in turns affects the dancing, thus providing ingredients for expressivity in a context of self-augmentation.  

The timing can be interpreted from a Bayesian perspective. The establishment of a timing model can be understood as a prior affecting the likelihood of synchronized events to occur in time. This prior can be adapted in view of past experiences and future anticipations, such as repeated phrases, and expression arcs that propulse to the future. The outcome is likely reflected in the *relative phase*, that is, the time difference between the musical event and the movement event, probably due to intuitions in which artistic expression is rooted.
...
A suitable music-dance representation is needed, using events extracted from music and dance. Then, a suitable statistical model for music-dance events has to be developed, such that phrases can be contrasted with each other in terms of sychronization differences.
...
Dancers were requested to perform 12 times the *Promenade in Arabesque* as an isolated figure (I.F1), followed by the entire Odile variation isolation in which F1 and F2 are embedded. 
...
Of these 12 trials, we exclude trial 5 and trial 8 because these trials entailed a manipulation that is not relevant in the present study. 
...
The musical event is the beat-onset which, in this study, is measured manually by experts.
...
The column indicated as `phase` holds the relative phase, that is, the time of the heel event relative to the time of the previous musical beat event in a context defined by the next musical beat event. This is expressed in a range of $[0,2\pi]rad$, see formula
...
Then we have `Fragment`, `Participant`, `Heel`, and `Trialf`. 
...
The vertical axis is again the phase of the heel-down event relative to the musical beat event, expressed in radians. Note dat the vertical lines are slightly tilted to the right. That's because time and relative phase are related. 


<div class="figure" style="text-align: center">
<img src="Figures/chapDancer_Dataplot_p3.png" alt="Plot of heel-down events of three different dancers for fragment F1, (top) one single trial, (bottom) all trials" width="100%" /><img src="Figures/chapDancer_Dataplot_p4.png" alt="Plot of heel-down events of three different dancers for fragment F1, (top) one single trial, (bottom) all trials" width="100%" />
...
Overall, the visualisation of the data suggests that our hypotheses could probably be confirmed by solid quantitative tests using statistical modelling.
...
This analysis is similar to an ANOVA, except that our data is circular, and that we use a multilevel distributional approach in the sense that we have a group-level variable `Trialf` and that we model the parameters of the `von Mises` response distribution model. Additionally, in the next section, we also add time.
...
The second line says that we have `kappa` as response, using the same specification of predictors as in the top part. Given a circular response distribution, the link function is *von_mises*, and the parameter corresponding to the gaussian `sigma` is called `kappa`.
It is defined as:
$$
...
Figure \@ref(fig:chapDancermodel1Fragments) shows the result of applying the model to the dataset. The cloud of points is just from the data. The model is specified in terms of the big dots and the error bars. They represent the predicted means and the critical interval (CI-95%) of these means, per group. 
A CI-95% means that estimates of the mean fall in this interval in 95% of the cases.

<div class="figure" style="text-align: center">
<img src="Figures/chapDancer_model1_Fragments.png" alt="Mean and variance of heel conditions in fragments" width="100%" />
<p class="caption">(\#fig:chapDancermodel1Fragments)Mean and variance of heel conditions in fragments</p>
</div>

In the next figure \@ref(fig:chapDancermodel1FragmentsParticipants) we go into a bit more detail by looking at each participant in the fragments. 

We also plot two error bars. 
...
The difference gives an indication of the importance of this group-level effect which, in this case, represents the effect of repeated measures. We are now ready to calculate the contrasts in order to obtain a quantitative picture of the difference among fragments.
...
To retrieve these posterior predictive distributions we use the `brms` function `epred_draws()`. It uses the fitted model and a dataset to generate draws from the expected value of the posterior distribution. We then use the R-package `circular` to calculate the mean and the standard deviation of the circular scale, and CI-95% from the standard deviation.
...
On top, we have the posterior predictive distributions of the mean for both `seg1` and `seg2`, standing for I.F1 and F1, respectively. 

In order to get distributions with an equal number of samples -- which we need for subtracting both distributions (see below) -- we define the two counterfactual datasets with an equal number of time points in the segment, regardless of segment duration of the original data.
...
Accordingly, the fitted model can be used to generalize over the specified time points, and thus create the wanted predictions. Details are provided in the code of script 06.
...
For `seg1`, the mean is about $0.44 [rad]$, while for `seg2`, it is about $0.21 [rad]$. Subtracting the distributions implies that we have an equal amount of draws in both distributions and that we take random samples from both, which we subtract.
This gives a new distribution `seg1 - seg2`, also radians, indicating a contrast between `seg1` and `seg2.` 
...
<div class="figure" style="text-align: center">
<img src="Figures/chapDancer_ContrastFragments_test_1.png" alt="Contrasts" width="100%" />
<p class="caption">(\#fig:chapDancerContrastFragmentsFigureA)Contrasts</p>
</div>
...
The calculated model comes with the code of this chapter.
...
Figure \@ref(fig:chapDancermodel3FragmentsParticipants) now shows the heel events as coloured dots, with lines indicating the mean over time, and grey bands indicating the uncertainty of the mean (95%-CI). 
...
Figures \@ref(fig:chapDancerContrastFigure1)
show the contrasts of two segments, with time intervals identified as `seg1` = $[2.51, 4.25]sec$ and `seg2` = $[4.26, 6.00]sec$ in fragment F2 and for heels-down.
...
These findings are in support of hypothesis 2, that phrases within fragments are different, because we assume that a prediction model will influence the micro-timing allowing for more anticipation.

Note that a key aspect of the computation is based on drawing posterior predictive distributions such that segments can be compared on the basis of an equal number of samples spread over the segment.
With `data_grid()` it is possible to generate an evenly spaced grid of new data points using the model. Even if the segments have different intervals of time, or if they would have missing data points, this grid can be used to draw posterior predictive distributions, which can then be compared with each other.

## Conclusion

Given our current knowledge about sensory-motor synchronization, memory and anticipation/prediction, contrast analysis seems to support the idea that a sequence establishes a micro-timing model in the dancer. When the sequence is repeated shortly after, the established micro-timing model facilitates the timing events for that sequence, which translates to a subtle shift in relative phase, in a direction suggesting increase in anticipation. Statistical modelling allows for dealing with circularity in the response (relative phase in radians) and having a multi-level representation of repeated measures nested within subjects with different variances in conditions. The resulting model could be called *circular-linear multi-level distributional smooth regression* but that's too long for a name, isn't it?
